...
var ModalInput = require('./ModalInput.js');
require('./public/js/opentok-client-2.4.1.min');

var TokTokView = function(state, user, sessionId, token, service) {
...
TokTokView.prototype = {
  ...
  createView: function() {
    ...
    var add = $.parseHTML('<div class="btn add-btn">Search user</div>')[0];
    ...
    self.$add.on('click', function(event) {
      if (_.isUndefined(self.searchUser)) {
        console.log('Searching friend...');

        function userFoundHandler(signal) {
          return function(username) {
            var signalData = JSON.parse(signal.data);
            if (signal.type == 'signal:founduser' &&
              signalData.username == username) {
              //TODO implement add to user cache model
              //TODO
              //TODO
              //TODO
              //TODO
        ...
        var placeholder = 'Type user alias...';
        var buttonMsg = 'Search';
        var userRegex = /^[a-zA-Z0-9_]{1,15}$/;
        self.searchUser = new ModalInput(userRegex, placeholder, buttonMsg,
          self.service.searchUser(userFoundHandler), false,
          self.service.cancelSearchUser(userFoundHandler));
      } else self.searchUser.toggle();
    });
  },
